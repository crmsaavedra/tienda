...
const { imageUrlFor } = require('./part-image');
const items = [
...
].map(([sku,name,brand,category,price,stock,description]) => ({ sku,name,brand,category,price,stock,description,lowStockThreshold:5,technicalSpecs:[{label:'Garantía',value:'6 meses'},{label:'Calidad',value:'Equipo equivalente OEM'}],compatibility:[{make:'Toyota',model:'Corolla',yearFrom:2015,yearTo:2023}] }));
items.forEach(item => { item.image = imageUrlFor(item); });
