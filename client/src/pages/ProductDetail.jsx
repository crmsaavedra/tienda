...
import { useCartStore, useAuthStore, useWishlistStore, useRecentStore } from '../store/store';
import { ChevronRight, Package, Tag, Info, AlertTriangle, CheckCircle, Star, Bell, Heart } from 'lucide-react';
...
export default function ProductDetail() {
  ...
  useEffect(() => {
    const fetchPart = async () => {
      try {
        ...
        recent.add(id);
        setMeta(
          `${res.data.name} — Autopartes Pro`,
          `Cotiza y compra ${res.data.name} (SKU ${res.data.sku}). Precio $${res.data.price.toLocaleString('es-CL')}. Envío a todo Chile.`
        );
    ...
    fetchPart();
    return () => setMeta('Autopartes Pro — Repuestos y Autopartes', 'Repuestos y autopartes de calidad para tu vehículo.');
  }, [id]);
  ...
  const placeholder = 'https://images.unsplash.com/photo-1555617781-db47da4fef27?w=800&q=80';
  const galleryImages = part.gallery?.length
    ? part.gallery
    : [part.image || placeholder];
  const activeImage = selectedImage || part.image || galleryImages[0] || placeholder;
              ...
              <div className="flex gap-2 mt-4 justify-center flex-wrap">
                {galleryImages.map((img, i) => (
                  <button
                    ...
                    className={`w-16 h-16 rounded-lg overflow-hidden border-2 transition-colors ${img === activeImage ? 'border-primary' : 'border-slate-200 hover:border-slate-400'}`}
                  >
                    <img src={img} alt={`${part.name} ${i + 1}`} className="w-full h-full object-cover" />
                  </button>
                ))}
              </div>
            )}
          ...
          <div className="p-8 sm:p-10 lg:pl-0">
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex items-center gap-2">
                ...
                >
                  <Heart className={`w-5 h-5 ${wishlist.has(part._id) ? 'fill-red-500 text-red-500' : 'text-slate-400 hover:text-red-400'}`} />
                </button>
              </div>
            </div>
            ...
            {part.stock > 0 ? (
              ...
              <p className="text-sm text-red-500 flex items-center font-medium mb-3">
                <AlertTriangle className="w-4 h-4 mr-1.5" /> Sin stock temporalmente
              </p>
            )}

            {part.stock <= 0 && !alertSent && (
              <div className="p-4 bg-orange-50 border border-orange-200 rounded-lg mb-4">
                <p className="text-sm font-bold text-orange-800 mb-2 flex items-center">
                  <Bell className="w-4 h-4 mr-1.5" /> Avísame cuando vuelva el stock
                </p>
                <form className="flex gap-2" onSubmit={async (e) => {
                  e.preventDefault();
                  try {
                    await axios.post('/api/stock-alerts', { partId: id, email: alertEmail || user?.email });
                    setAlertSent(true);
                    toast.success('Te avisaremos cuando este producto esté disponible.');
                  } catch (err) {
                    toast.error(err.response?.data?.error || 'No se pudo registrar tu aviso');
                  }
                }}>
                  <input
                    type="email" required value={alertEmail || user?.email || ''}
                    onChange={(e) => setAlertEmail(e.target.value)}
                    placeholder="tu@email.com"
                    className="flex-1 px-3 py-2 border border-slate-300 rounded-md focus:ring-primary focus:border-primary text-sm"
                  />
                  <button type="submit" className="bg-orange-600 hover:bg-orange-700 text-white px-4 py-2 rounded-md text-sm font-bold transition-colors">
                    Avisarme
                  </button>
                </form>
              </div>
            ...
            {part.stock <= 0 && alertSent && (
              <p className="text-sm text-green-600 flex items-center font-medium mb-3">
                <CheckCircle className="w-4 h-4 mr-1.5" /> ¡Listo! Te avisaremos por correo.
              </p>
            )}
          </div>
        </div>
