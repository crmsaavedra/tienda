...
export default function AdminOrderDetail() {
  ...
  const handleDownloadInvoice = async () => {
    try {
      const res = await axios.get(`/api/admin/orders/${order._id}/invoice`, {
      ...
      link.href = url;
      link.setAttribute('download', `Boleta-${order._id.toString().slice(-6)}.pdf`);
      ...
      toast.error(err.response?.data?.error || 'Error al descargar la boleta');
    }
  };
            ...
            <div className="space-y-4">
              {order.items?.map((item, i) => (
                <div key={item.part || i} className="flex gap-4 items-center">
                  <img src={item.image || `/uploads/${item.sku}.svg`} alt={item.name} className="w-16 h-16 object-cover rounded-lg border border-slate-200" />
                  <div className="flex-1">
                    <p className="font-bold text-slate-900 line-clamp-1">{item.name}</p>
                    <p className="text-sm text-slate-500">Cantidad: {item.qty} x ${(item.unitPrice || 0).toLocaleString('es-CL')}</p>
                  </div>
                  <div className="font-bold text-slate-900">
                    ${((item.qty || 0) * (item.unitPrice || 0)).toLocaleString('es-CL')}
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-6">
            ...
            {(order.status === 'paid' || order.status === 'dispatched' || order.status === 'delivered') && (
              <button onClick={handleDownloadInvoice} className="mt-4 block w-full text-center bg-slate-100 hover:bg-slate-200 text-slate-800 font-bold py-2 rounded transition-colors">
